...
Given a actual wiget name with description: {prompt}.
The description is what the widget needs to achieve. Generate the react component compatible with Tailwind CSS and shadcn, accroding to the name and description, you may only access to Tailwind CSS and shadcn dependencies.
The content props should be well defined to reflect on the purpose of the widget, don't be overly simple or complicated.


The const componentname shoud match the actual wiget name.

return in this format {react}

The type name should be assigned to the name in the prompt, i.e. with "-" in between.
...
def react_refine(response, react, prompt):
    body = json.dumps({
        "max_tokens": 3000,
        "messages": [
            {
...
Imagen You are the best Web Designer in the whole fucking universe.
Here is what you gonna do:
Given a actual wiget: {response}.
Convert the widget in this format {react} meanwhile remain the visuals and compatibility with Tailwind and Shadcn. And give a real example data in the demo data

The type name should be assigned to the name in the prompt, i.e. with "-" in between, example: chart-linchart-labels.
...
def batch_generate_reacts(prompts, react):
    ...
    with ThreadPoolExecutor() as executor:
        future_to_prompt = {executor.submit(generate_react, prompt, react): prompt for prompt in prompts}
